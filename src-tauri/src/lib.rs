...
async fn activate_customer(request: ActivationRequest) -> ActivationResult {
    if request.code.trim().is_empty() || request.code.trim().len() > 64 || !(16..=256).contains(&request.device_id.trim().len()) || !valid_account_id(&request.account_id) {
...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![activate_customer, delete_customer_session])
